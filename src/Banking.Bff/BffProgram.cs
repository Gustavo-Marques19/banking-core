...
/// <summary>
/// Backend-for-frontend (ADR-011). Faz o login OIDC, guarda os tokens no servidor e entrega ao navegador só um cookie
/// de sessão. /api é repassado à API com o token do usuário. Cada front roda a sua instância, com o próprio cliente
/// OIDC, cookie e papéis permitidos: o backoffice para operador e admin, o app para cliente.
/// </summary>
public static class BffProgram
{
    ...
    public static void Main(string[] args)
    {
        ...
        if (!options.CookieName.StartsWith("__Host-", StringComparison.Ordinal) || options.AllowedRoleList.Length == 0)
        {
        ...
        builder.Services.AddAuthorizationBuilder()
            .AddPolicy(AppPolicy, policy => policy.RequireAuthenticatedUser().RequireRole(options.AllowedRoleList))
            .SetFallbackPolicy(new AuthorizationPolicyBuilder().RequireAuthenticatedUser().RequireRole(options.AllowedRoleList).Build());

        var routes = new List<RouteConfig>
        {
            new() { RouteId = "api", ClusterId = "api", AuthorizationPolicy = AppPolicy, Match = new RouteMatch { Path = "/api/{**rest}" } },
        };
